...
The two methodologies involve different trade-offs. The original AHS2023 methodology, based on a parametric distance gradient, tends to generate more local variation and may therefore capture highly localized market responses more sharply, particularly in thin markets with large effective catchment areas. At the same time, this approach can be more sensitive to influential outliers and may occasionally generate pronounced movements in individual years driven by a small number of observations. In contrast, the AHSY methodology produces smoother indices that are generally less sensitive to influential outliers due to the use of weighted local catchment areas and adaptive radius selection. However, this smoothing may reduce responsiveness to highly localized market dynamics in areas with sparse transaction activity.
...
| `APPLICATIONS` |  `DATA/OUTPUT/2025` | 2025 edition of the postcode indices including years 2007-2024 | This folder contains indices computed based on **real** data |
| `APPLICATIONS` |  `DATA/OUTPUT/2026` | 2026 edition of the postcode indices including years 2007-2025 | This folder contains indices computed based on **real** data |
